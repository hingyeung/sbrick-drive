...
var SBrick = function() {
	...
	this.drive = function(channels, direction, power) {
		var command = [DRIVE_CMD];
		for (var idx = 0; idx < NUM_CHANNEL; idx++) {
			var commandForThisChannel = [
				idx, direction,
				_.includes(channels, idx) ?
					power :
	...
	this.break = function(channels) {
		// break all command
		var command = _.concat(BREAK_CMD, channels);
		writeWithoutResponse(Buffer.from(command));
	};

	this.stop = function(channels) {
		this.drive(
			channels,
			SBCommand.DIRECTION.FORWARD,
			SBCommand.POWER_SETTING.NO_POWER);
	};
